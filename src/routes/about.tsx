...
function About() {
  return (
    <SiteLayout>
          ...
          <div className="lg:col-span-5 grid gap-6">
            <div className="rounded-4xl border border-brand/10 bg-brand-soft p-8 shadow-(--shadow-soft)">
              <div className="flex items-start gap-6">
                <div className="flex h-24 w-24 items-center justify-center rounded-3xl bg-paper border border-brand/20 shadow-lg">
                  <img src={logo} alt="PICDECOS logo" className="h-14 w-14 object-contain" />
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.25em] text-brand font-bold">Brand area</p>
                  <h3 className="font-serif text-3xl mt-3">Logo ready to update later.</h3>
                  <p className="mt-3 text-sm text-muted-foreground">
                    This panel leaves room for the cooperative logo and brand mark, so the page stays polished and flexible.
                  </p>
                </div>
              </div>
            </div>
            <div className="rounded-4xl border border-border bg-background p-8 shadow-(--shadow-soft)">
              <p className="text-xs uppercase tracking-[0.25em] text-brand font-bold mb-3">Green first</p>
              <h3 className="font-serif text-3xl">Warm, clean, and easy to update.</h3>
              <p className="mt-4 text-sm text-muted-foreground leading-relaxed">
                The layout is intentionally calm and focused on the cooperative's people, products, and mission. You can replace the logo image at any time without changing the structure.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 px-6 bg-brand-soft">
        <div className="mx-auto max-w-7xl rounded-4xl border border-brand/10 bg-background p-8 shadow-(--shadow-soft)">
          <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] items-center">
            <div className="rounded-[1.75rem] border border-brand/10 bg-brand/10 p-8 text-center">
              <p className="text-xs uppercase tracking-[0.3em] text-brand font-semibold mb-4">Catalogue-ready</p>
              <div className="mx-auto mb-6 h-32 w-32 rounded-3xl bg-brand-soft/90 flex items-center justify-center text-brand text-lg font-semibold">
                Product shots
              </div>
              <p className="text-sm text-muted-foreground leading-relaxed">
                When you add product images, this cooperative catalogue will look strong, professional, and easy to browse.
              </p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-[0.25em] text-brand font-bold mb-3">Ready for your logo</p>
              <h3 className="font-serif text-3xl">The page is built to accept your brand identity.</h3>
              <p className="mt-4 text-sm text-muted-foreground leading-relaxed">
                Use the logo image above or replace the source file later. The layout stays consistent across desktop and mobile while keeping the design bright and green.
              </p>
            </div>
          </div>
        </div>
